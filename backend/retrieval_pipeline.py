...
class OpenRouterEmbedder:
    ...
    def embed_query(self, text: str) -> np.ndarray:
        ...
        try:
            ...
            raise RuntimeError(f"OpenRouter embeddings request failed: {e}") from e

        if response.status_code != 200:
            body_preview = response.text[:500] if response.text else ""
            raise RuntimeError(
                f"OpenRouter embeddings error {response.status_code}: {body_preview}"
            )
        ...
        try:
            ...
            raise RuntimeError(
                f"OpenRouter embeddings response was not valid JSON: {response.text[:500]}"
            ) from e
        ...
        try:
            ...
            raise RuntimeError(
                f"OpenRouter embeddings response missing expected fields: {response_data}"
            ) from e
...
class RetrievalPipeline:
    ...
    def __init__(self, config: Optional[RetrievalPipelineConfig] = None):
        ...
        self.embedder = OpenRouterEmbedder(config.openrouter_api_key, model=config.embedding_model)
        ...
        for r in bm25_results:
            ...
            if chunk_id in combined:
                ...
                combined[chunk_id]["combined_score"] = (
                    self.config.semantic_weight * combined[chunk_id].get("semantic_score", 0)
